  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const { data: requests } = await supabase
        ...
        .select("id,number,status,priority,description,quantity,unit,needed_by,created_at,decided_at,purchased_at,arrived_at,sectors(code,name),items(code,description)");
      return requests ?? [];
    },
  });
